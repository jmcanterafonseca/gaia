...
var GlobalContacts = (function GCDSOps() {
  ...
  var remove = function remove(originStore, originDsId, contact) {
    ...
    return new Promise(function(resolve, reject) {
      getDatastore().then(function success(store) {
        globalDsId = index.byStore[originStore.owner][originDsId];
      ...
      }, reject).then(function onEntry(entry) {
          if (!Array.isArray(entry)) {
            reject();
            return;
          }
          doRemove(entry, originStore,
  ...
  function doRemove(entry, originStore, originDsId, globalDsId, contact) {
    return new Promise(function(resolve, reject) {
      var position = -1;
      entry.forEach(function onEntry(aEntry, i) {
      ...
      if (entry.length === 0) {
        ...
        store.put(entry, globalDsId).then(resolve, reject);
      }
    });
  }
