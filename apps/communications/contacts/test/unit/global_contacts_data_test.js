...
suite('Global Contacts Data', function() {
  ...
  test('Adding a new Contact to the datastore', function(done) {
    GlobalContacts.add(originStore1, '1234', contact).then(function() {
  ...
  test('New Contact matches existing > entry is updated ', function(done) {
    ...
    GlobalContacts.add(originStore2, '678', aMatchingContact).then(
      function success() {
        done(function() {
          var records = MockDatastore._records;
  ...
  test('New Contact does not match existing > New entry is created',
    function(done) {
      GlobalContacts.add(originStore2, '9999', anotherContact).then(function() {
      return GlobalContacts.flush();
    }, function reject(error) {
        done(error);
    }).then(function success() {
  ...
  test('Remove datastore entry. Entry remains ', function(done) {
    GlobalContacts.remove(originStore2, '678', aMatchingContact).then(
      function success() {
        return GlobalContacts.flush();
      }, function reject(error) {
          done(error);
  ...
  test('Remove datastore entry. Entry is deleted ', function(done) {
    GlobalContacts.remove(originStore2, '9999', anotherContact).then(
      function success() {
        return GlobalContacts.flush();
      }, function reject(error) {
          done(error);
  ...
  test('Clear the datastore', function(done) {
    GlobalContacts.clear(originStore1).then(function success() {
      return GlobalContacts.flush();
    }, function reject(error) {
        done(error);
    }).then(function success() {
