...
function MultiContact(sourceRawData, _multiContactData) {
  this._sourceRawData = sourceRawData;
...
MultiContact.prototype = {
  save: function() {
    var multiContactData = this._multiContactData;
    ...
    return new window.Promise(function(accept, reject) {
      ...
      if(sourceRawData)  {
        var primaryData = this._getPrimaryData();
        var savedData = {
          data: primaryData,
          sources: []
        };
        sourceData.forEach(function(aSource) {
          savedData.sources.push({
            source: aSource.source,
            id: aSource.data.id
          });
        });
      }
      MultiContacts.save(savedData).then(accept, reject);
    });
